  ...
  const TransactionProgress = () => (
    <div className="mb-6 space-y-4">
        ...
        <div>
          ...
          {approvalHash && (
            <p className="text-xs text-gray-500 mt-1">
              TX: {approvalHash.substring(0, 12)}...
              {approvalHash.substring(approvalHash.length - 4)}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 ${
            transactionStep === "purchasing" || transactionStep === "completed"
              ? "bg-emerald-500 text-white"
              : "bg-gray-200"
          }`}
        >
        ...
        <div>
          ...
          {purchaseHash && (
            <p className="text-xs text-gray-500 mt-1">
              TX: {purchaseHash.substring(0, 12)}...
              {purchaseHash.substring(purchaseHash.length - 4)}
            </p>
          )}
        </div>
      </div>
        ...
        <div className="p-6">
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Quantity (Available:{" "}
              {commodity.commodityQuantity.toLocaleString()})
            </label>
            {/* <input
              type="number"
              min="1"
